...
describe('DatepickerComponent', () => {
  ...
  const getSelectedDate = (): Date => {
    const selectedDate = component.selectedDate.value;

    if (selectedDate === null) {
      throw new Error('Expected selectedDate to contain a date.');
    }
  ...
  const queryButton = (selector: string): HTMLButtonElement => {
    ...
    if (!(element instanceof HTMLButtonElement)) {
      throw new Error(`Expected ${selector} to match a button element.`);
    }
  ...
  it('should disable the prev button in the DOM when at minDate', () => {
    ...
    const prevBtn = queryButton('.prev-btn');
  ...
  it('should disable the next button in the DOM when at today', () => {
    ...
    const nextBtn = queryButton('.next-btn');
