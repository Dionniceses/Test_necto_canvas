...
export class DatepickerComponent {
  ...
  readonly selectedDateValue = input<Date | null>(null);
  ...
  constructor() {
    effect(() => {
      const incomingDate = this.selectedDateValue();
      const currentDate = this.selectedDate.value;

      if (incomingDate) {
        if (!currentDate || incomingDate.getTime() !== currentDate.getTime()) {
          this.selectedDate.setValue(incomingDate, { emitEvent: false });
          this.#selectedDateSignal.set(incomingDate);
        }
      }
    });
